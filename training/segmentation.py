...
class Segmentation(lightning.LightningModule):
    ...
    def train_dataprep(self, batch):
        ...
        if torch.rand(1) < 0.5:
            ...
            sourceds_ignore_mask = FV.hflip(sourceds_ignore_mask)

        sourceds_color_aug_image = self.random_aug_weak(sourceds_image.detach().clone().float() / 255.)

        if self.use_strong_aug_source:
            sourceds_color_aug_image = self.random_aug_strong(sourceds_image.detach().clone().float() / 255.)
    ...
    def training_step(
        ...
        self.log("train_loss", loss_source, prog_bar=True)
        ...
        with torch.no_grad():
            ...
            self.log("train_acc", acc_source, prog_bar=True)
